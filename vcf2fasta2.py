...
def extractvcf_phased(data, var, head, sampstart, samples, gff):
    if gff.get(var[0]):
...
def extractvcf_unphased(data, var, head, sampstart, samples, gff):
    if gff.get(var[0]):
        gene = getgene(var, gff)
        if gene:
            data = startdict(data, gene)
...
def extractvcf_haploid(data, var, head, sampstart, samples, gff):
    if gff.get(var[0]):
        gene = getgene(var, gff)
        if gene:
            data = startdict(data, gene)
